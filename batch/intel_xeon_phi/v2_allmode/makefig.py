...
def makefig_one(ch, coord, result, result_s, topo, topo_idxnum):
	...
	#title
	font = ImageFont.truetype(font='DejaVuSans.ttf',size=16)
	...
	#min,max,delta
	font = ImageFont.truetype(font='DejaVuSans.ttf',size=16)
	draw.text((coord[0]+w*0.8, coord[1]+h*9.2), 'min = '+str(cmax), fill=(0, 0, 0), font=font)
	draw.text((coord[0]+w*0.8, coord[1]+h*10.2), 'max = '+str(cmin), fill=(0, 0, 0), font=font)
	draw.text((coord[0]+w*6.5, coord[1]+h*9.6), 'delta = '+str(cmax-cmin)+' (ns)', fill=(0, 0, 0), font=font)
