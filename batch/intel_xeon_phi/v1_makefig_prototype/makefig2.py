...
def makefig_one(memnum, coord):
	...
	#numanode: ((0,18),(18,36),(36,52),(52,68))
	...
	df2 = pandas.read_csv('snedo01_cpuinfo_arranged', header=None)
	#print(df2)
	#print(df2[df2[:][1] == 61].iloc[0,0])
	#print((df2[:][1] == 61).any())
	#sys.exit()
	...
	for i in all_coord:
		if (df2[:][1] == count).any():
			processor = df2[df2[:][1] == count].iloc[0,0]
			if (df[:][0] == processor).any():
				#print(df[df[:][0] == processor].iloc[0,0])
				color = coloring(df[df[:][0] == processor].iloc[0,1],cmin,cmax)
				s = str(processor)
			else:
				pass
				#processor 14
